...
def main():
    ...
    for data, target in test_loader:
        ...
        noisy_data = torch.add(
            data.data, random_noise_size, torch.randn(data.size()).cuda()
        )
        ...
        if args.adv_type == "FGSM":
            ...
            if args.net_type == "densenet":
                ...
                gradient.index_copy_(
                    ...
                    gradient.index_select(1, torch.LongTensor([1]).cuda())
                    / (62.1 / 255.0),
                )
                gradient.index_copy_(
                    1,
                    torch.LongTensor([2]).cuda(),
                    gradient.index_select(1, torch.LongTensor([2]).cuda())
                    / (66.7 / 255.0),
                )
            else:
                gradient.index_copy_(
        ...
        if args.adv_type == "DeepFool":
            ...
            _, adv_data = adversary.cw(
                model, data.data.clone(), target.data.cpu(), 1.0, "l2", crop_frac=1.0
            )
        else:
    ...
    torch.save(
        clean_data_tot,
        "%s/clean_data_%s_%s_%s.pth"
        % (args.outf, args.net_type, args.dataset, args.adv_type),
    )
    torch.save(
        adv_data_tot,
        "%s/adv_data_%s_%s_%s.pth"
        % (args.outf, args.net_type, args.dataset, args.adv_type),
    )
    torch.save(
        noisy_data_tot,
        "%s/noisy_data_%s_%s_%s.pth"
        % (args.outf, args.net_type, args.dataset, args.adv_type),
    )
    torch.save(
        label_tot,
        "%s/label_%s_%s_%s.pth"
        % (args.outf, args.net_type, args.dataset, args.adv_type),
    )
