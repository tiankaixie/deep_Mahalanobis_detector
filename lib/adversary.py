...
def fgs_search(
    ...
    for i in range(precision):
        ...
        for exponent in exponents:
            ...
            if verbose:
                print(
                    "step size = %1.4f,  success rate = %1.4f"
...
def fgs_compute_status(
    model, inputs, outputs, targets, status, batch_size=25, threshold=0.9, verbose=True
):
...
def deepfool_single(
    ...
    for m in range(max_iter):
        imgs_var_in = imgs_var2.expand(
            1, imgs_var2.size(0), imgs_var2.size(1), imgs_var2.size(2)
        )
        ...
        r = r.add(ri.data)
        imgs_var_in = imgs_var2.clone().expand(
            1, imgs_var2.size(0), imgs_var2.size(1), imgs_var2.size(2)
        )
        ...
        if verbose:
            print(
                "iteration " + str(m + 1) + ": perturbation norm ratio = " + str(diff)
            )
        if pred2 != cpu_targets:
            if verbose:
                if labels:
                    print(
                        "old label = %s, new label = %s"
                        % (labels[cpu_targets], labels[pred2])
                    )
                else:
    ...
    for i in range(max_iter):
        ...
        if crop_frac < 1 and i % 3 == 1:
            ...
            for m in range(w.size(0)):
                ...
                grad_m = to_tensor(
                    scale_down(to_pil((grad_cpu[m] - minimum) / maximum))
                )
...
class _MahalanobisEnsembleLoss(nn.Module):
    ...
    def forward(self, feature, mean, inverse_cov, weight, top2_index):
        ...
        for i in range(len(mean)):
            ...
            temp_loss = torch.mm(
                torch.mm(zero_f, Variable(inverse_cov[i])), zero_f.t()
            ).diag()
            mahalaonbis_loss += weight[i] * torch.mean(temp_loss)
        return mahalaonbis_loss


def attacking_mahalanobis(
    model,
    input,
    target,
    weight,
    ...
    for i in range(max_iter):
        ...
        for j in range(len(out_features)):
            out_features[j] = out_features[j].view(
                out_features[j].size(0), out_features[j].size(1), -1
            )
        ...
        if verbose:
            print(
                "iteration %d: loss = %f, %s_loss = %f, "
                "adv_loss = %f" % (i + 1, total_loss, loss_str, recons_loss, adv_loss)
            )
            print("robustness = %f, success rate = %f" % (rb, sr))
        if total_loss < best_loss:
            best_loss = total_loss
...
def attacking_mahalanobis_all(
    ...
    for i in range(max_iter):
        ...
        temp_loss = mahalanobis_criterion(
            out_features, sample_mean, inv_sample_conv, ensemble_weight, top2_index
        )
