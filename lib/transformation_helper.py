...
def setup_transformations(args, data_type, defense, crop=None):
    ...
    if "adversary" not in args or args.adversary is None:
        if data_type == "train":
            ...
            if "preprocessed_data" not in args or not args.preprocessed_data:
                transform.append(
                    torch_trans.Scale(args.data_params["IMAGE_SCALE_SIZE"])
                )
                transform.append(torch_trans.CenterCrop(args.data_params["IMAGE_SIZE"]))
            transform.append(torch_trans.ToTensor())
            if crop:
        ...
        if (
            "preprocessed_data" in args
            and not args.preprocessed_data
        ...
        transform.append(
            transforms.Scale(
                args.data_params["IMAGE_SIZE"], args.data_params["MEAN_STD"]
            )
        )
    ...
    if "normalize" in args and args.normalize:
        transform.append(
            torch_trans.Normalize(
                mean=args.data_params["MEAN_STD"]["MEAN"],
...
# Update dataset
def update_dataset_transformation(dataset, args, data_type, defense, crop):
